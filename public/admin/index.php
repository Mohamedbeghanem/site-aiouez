...
?>
    <div class="page-actions"><div class="task-toolbar"><div class="filter-chips"><?php foreach(['open'=>'À faire','completed'=>'Terminées','all'=>'Toutes'] as $key=>$label): ?><a class="<?= $statusFilter===$key?'active':'' ?>" href="?view=tasks&status=<?= $key ?>&mode=<?= e($mode) ?>"><?= $label ?></a><?php endforeach; ?></div><div class="view-switch"><a class="<?= $mode==='list'?'active':'' ?>" href="?view=tasks&mode=list&status=<?= e($statusFilter) ?>">Liste</a><a class="<?= $mode==='calendar'?'active':'' ?>" href="?view=tasks&mode=calendar&status=<?= e($statusFilter) ?>">Calendrier</a></div></div><a class="primary-button" href="?view=tasks&new=1">+ Nouvelle tâche</a></div>
